...
- **Installed Layout Rule:** In repositories where IteraSpec has been installed under `.iteraspec/`, the protected GUI directory may be `.iteraspec/gui/` instead of root `gui/`. The AI must protect whichever repository path is actually used by the local IteraSpec installation and must not assume that only one layout exists across all repositories.
